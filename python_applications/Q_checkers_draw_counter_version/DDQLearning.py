...
class DDQLearning(object):
    def __init__(self, agt_name):
        self.agent = DDQNAgent(agt_name, True)
    ...
    def auto_play(self, n_episodes):
        for i in range(n_episodes):
            ...
            while (not gm.is_over()):
                ...
                move_idx, q_val = Gameplay.get_QAgent_move_pp(self.agent, gm)
            ...
            for k, v in turns_hist.items():
                for turn_hist in v:
                    self.agent.remember(turn_hist['board_state'], turn_hist['draw_counter'],
